...
function createWebtaskCreator(args, options) {
    ...
    function printCode(outfile, code, meta) {
        return Promise.all([
            new Promise(function(resolve, reject) {
                Fs.writeFile(outfile, code, 'utf8', function (err) {
                    if (err) {
                        exitError('Error writing output to `' + Path.basename(outfile) + '`: ' + err.message);
                        reject();
                    }

                    // eslint-disable-next-line no-console
                    console.log('Bundle successfully written to `%s`', outfile);
                    resolve();
                });
            }),
            new Promise(function(resolve, reject) {
                Fs.writeFile(`${outfile}.meta.json`, JSON.stringify(meta, null, 4), 'utf8', function (err) {
                    if (err) {
                        exitError('Error writing metadata file to `' + Path.basename(`${outfile}.meta.json`) + '`: ' + err.message);
                        reject();
                    }

                    // eslint-disable-next-line no-console
                    console.log('Metadata file successfully written to `%s`', `${outfile}.meta.json`);
                    resolve();
                });
            }),
        ])
    ...
    function createBundledWebtask(profile, nodeVersion) {
        ...
        function onGeneration(build) {
            ...
            return args.outfile
                ? printCode(args.outfile, build.code, args.meta)
                : putWebtask(profile, args, build.code)
            ...
            function onWebtaskError(generation, err) {
                if (lastGeneration === generation) {
                    // eslint-disable-next-line no-console
                    console.error('onWebtaskError', err.stack);
                    throw err;
                }
            }
        }
